...
def plotGlobalResponse(OutputDirName, AnalysisName):
    ...
    reaction3FileName = baseName + '_REACTIONS_3.out'
    ...
    Node3Reac = np.loadtxt(reaction3FileName, delimiter= ' ')
    
    LatLoad = Node1Reac[:,1] + Node2Reac[:,1] + Node3Reac[:,1]
    NodeLateralDisp = NodeDisp[:,1]


    fig, ax = plt.subplots()
    #plt.plot(NodeLateralDisp, -LatLoad, label=OutputDirName , linewidth=1)
    ax.plot(NodeLateralDisp, -LatLoad, label=OutputDirName , linewidth=1)
    ...
    # Mostrar el gráfico
    plt.show()
    #plt.savefig(baseName + '_Desp-ReacHoriz.png')
...
def plotLocalResponse(OutputDirName, AnalysisName):
    ...
    return epsyy_panel_1, sigyy_panel_1
