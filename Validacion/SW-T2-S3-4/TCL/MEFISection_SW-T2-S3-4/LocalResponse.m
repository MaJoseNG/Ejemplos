...
for j = 1:4
    Ele = 1;
    file = ['MEFISection_panel_strain' num2str(Ele) num2str(j) '.out'];
    panel_strain_Em(:,j*4-3:j*4) = importdata(fullfile(datafolder,file));
end


% Ayuda Jose
path = 'C:\repos\Ejemplos\Validacion\SW-T2-S3-4\TCL\MEFISection_SW-T2-S3-4\MEFISection_SW-T2-S3-4_RCLMS01_Conc02_Steel02_NUEVO_recordersHeightvsHorStrain_v2';

FolCon = dir(path);

% Hmod =  dir('C:\Users\joseb\OneDrive - Universidad de Chile\PHD 2023\Paper-NLSR\WCEE\Output');
cd([path])
for i=8:47  
        g(i-7).strain = importdata([FolCon(i).name]);
        g(i-7).data = [FolCon(i).name];    
end
